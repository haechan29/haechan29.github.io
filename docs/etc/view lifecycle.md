...
6. onDraw(): Canvas, Paint 등의 객체를 사용하여 텍스트, 비트맵, 도형 등 뷰의 시각적 내용을 그린다.<br/><br/>

&plus; invalidate(): 글자나 색상 등 크기 변화 없이 단순히 뷰의 속성 등이 변경되어 다시 그려야하는 경우 호출.<br/>
&plus; requestLayout(): 뷰의 크기가 변화할 경우 레이아웃의 배치도 달라질 수 있기 때문에 해당 메소드를 호출함으로써 뷰들의 크기부터 다시 측정.<br/>

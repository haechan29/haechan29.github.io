...
- 하나 이상의 스레드가 할당된다.<br/>
- 독립적인 메모리 공간(스택, 힙 데이터, BSS, 코드 영역)을 할당받는다.<br/>

### 스레드
- 시스템 자원을 할당받아 실행되는, 프로세스 내부의 작업 묶음.<br/>
- 프로세스가 할당받는 시스템 자원을 이용한다. 스택 영역을 제외한 메모리 공간을 할당받는다.<br/>

### 프로세스와 스레드의 메모리 구조
각각의 프로세스는 독립적인 메모리 공간을 할당받는다.<br/>

1. 스택<br/>
    - 함수 호출 시 생성되는 지역 변수, 매개 변수 등이 저장되는 영역. 함수 호출과 함께 할당되고 함수가 반환될 때 해제된다.<br/>
...
- ``프로세스``는 데이터를 격리하여 __안정성__ 면에서 유리한 반면, ``스레드``는 메모리 공간을 대부분 공유하기 때문에 [컨텍스트 스위칭의 비용이 낮고] [1], __작업 속도가 빠르다__.<br/>

...
## 안드로이드 앱은 어떻게 빌드되나요?
### 안드로이드 앱의 빌드 과정
1. Java 소스 코드 컴파일링: 개발자가 작성한 Java 소스 코드는 Java 컴파일러(javac)에 의해 바이트코드(.class 파일)로 컴파일된다. 이 바이트코드는 [Java 가상 머신(JVM)에서 실행] [1]될 수 있으며, 안드로이드 개발에서는 이후 단계에서 더 처리된다.<br/>
...
- [Dalvik] [2]은 JIT 컴파일을 사용하여 애플리케이션을 실행할 때마다 일정한 시간이 소요된다.<br/>
- [ART] [3]는 AOT 컴파일 방식을 사용하여 실행 속도가 빠르고, 시스템 효율성이 높다.<br/>
...
[3]: https://brunch.co.kr/@mystoryg/82

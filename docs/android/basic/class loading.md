...
### Class Loading
클래스가 처음 참조될 때 JVM의 메모리에 로딩된다. 아래의 과정을 거친다.<br/>
1. 로드(Load)
...
2. 링크(Link)
   - 로드된 클래스 파일들을 검증하고, 준비하며, 클래스 간의 상호 작용을 가능하게 하는 단계. 검증, 준비, 해석의 세 부분으로 나뉜다.<br/>
...
3. 초기화(Initialize)
   ...
   - 정적 변수들이 정의된 순서대로 초기화되고, [정적 블록] [1]이 실행된다<sup>2</sup>.<br/><br/>
